...
class WaveUI:
        ...
        if os.path.exists(self.metadata_file):
            try:
                ...
                if (
                    ...
                    and "report_id" in self.metadata_df.columns
                ):
    ...
    def add_metadata_entry(self, stage_params, prev_stage_params=None):
        ...
        self.metadata_df = pd.concat(
            [self.metadata_df, pd.DataFrame([new_row])], ignore_index=True
        )
    ...
    def launch_wave(self):
        ...
        try:
            ...
            self.app = Application(backend="uia").start(
                f'"{self.file_name}" "{self.project_path}"'
            )
            time.sleep(APPLICATION_LOAD_TIMEOUT)

            # Get the main window
            window_title = WINDOW_TITLE_PATTERN.format(
                project_name=self.project_name, case_name=self.case_name
            )
    ...
    def set_reverse_osmosis_parameters(
        ...
        try:
            logger.info(
                f"Setting RO parameters: PV={pv_per_stage}, ELS={els_per_pv}, Pressure={pressure} for stage {self.stages}"
            )
            ...
            try:
                ...
                radio_button = stages_group.child_window(
                    title=f"{self.stages}  ", control_type="RadioButton"
                )
            ...
            if self.stages == 1:
                ...
                logger.info(
                    f"Previous stage(s) values are {prev_stage_params} of format (pv, els, element_type pressure, *boost_pressure)"
                )
    ...
    def _handle_boron_notification_dialog(self):
        """
        Handles the Boron Notification dialog that appears when selecting NF200-4040 element type.
        Only runs once since dialog only appears on first NF200-4040 selection.

        Returns:
            bool: True if dialog was handled successfully, False if no dialog appeared or handling failed
        """
        # Skip if already handled in this session
        if hasattr(self, "_boron_dialog_handled") and self._boron_dialog_handled:
            return True
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    boron_dialog = Desktop(backend="win32").window(
                        title="Boron Notification"
                    )
                    wait_until_passes(5, 1, lambda: boron_dialog.exists())

                    if boron_dialog.exists():
                        logger.info(
                            f"Found Boron Notification dialog (attempt {attempt + 1})"
                        )

                        # Try multiple approaches
                        methods = [
                            lambda: boron_dialog.Button(title="OK").click(),
                            lambda: boron_dialog.Button(control_id=1).click(),
                            lambda: keyboard.send_keys("{ENTER}"),
                            lambda: keyboard.send_keys("%{F4}"),
                        ]
                        ...
                        for method in methods:
                            try:
                                method()
                                time.sleep(0.5)
                                if not boron_dialog.exists():
                                    logger.info(
                                        "Successfully closed Boron Notification dialog"
                                    )
                                    self._boron_dialog_handled = True
                                    return True
                            except Exception:
                                continue
            ...
            logger.warning(
                "Could not handle Boron Notification dialog after max retries"
            )
            self._boron_dialog_handled = True
            return False

        except Exception as e:
            logger.error(f"Error in background thread handling Boron dialog: {e}")
            self._boron_dialog_handled = True
            return False
    ...
    def export_to_excel(self, file_path):
        ...
        try:
            ...
            toolbar = self.main_window.child_window(
                auto_id=TOOLBAR_ID, control_type="ToolBar"
            )
            ...
            dropdown_item = self.main_window.child_window(
                title="Excel", control_type="MenuItem"
            )
    ...
    def _handle_exit_confirmation(self):
        ...
        try:
            ...
            try:
                confirm_dialog = Desktop(backend="win32").window(
                    title="Confirmation", class_name="#32770"
                )
            ...
            try:
                save_dialog = Desktop(backend="win32").window(
                    title_re="Save.*", class_name="#32770"
                )
                if save_dialog.exists():
                    ...
                    try:
                        no_button = save_dialog.child_window(
            ...
            try:
                ...
                logger.info(
                    f"Main window verification error: {e}, assuming closed successfully"
                )
    ...
    def get_pressure_from_excel(
        self, stage, pv, els, element_type, pressure, prev_stage_params=None
    ):
        """
        ...
        try:
            logger.info(
                f"Looking up feed pressure from Excel file: {self.prev_stage_excel_file}"
            )

            # Read the metadata sheet from the Excel file
            metadata_df = pd.read_excel(
                self.prev_stage_excel_file, sheet_name="Metadata"
            )
            ...
            metadata_df["current_stage_params"] = metadata_df[
                "current_stage_params"
            ].apply(lambda x: json.loads(x) if isinstance(x, str) else x)
            metadata_df["previous_stages_params"] = metadata_df[
                "previous_stages_params"
            ].apply(lambda x: json.loads(x) if isinstance(x, str) else x)
            ...
            for idx, row in metadata_df.iterrows():
                ...
                if (
                    cur_params.get("stage") == current_params["stage"]
                    and cur_params.get("pv") == current_params["pv"]
                    and cur_params.get("els") == current_params["els"]
                    and cur_params.get("element_type") == current_params["element_type"]
                ):

                    # Check appropriate pressure field based on stage
                    if stage == 1:
                        if (
                            cur_params.get("feed_pressure")
                            != current_params["feed_pressure"]
                        ):
                            continue
                    else:
                        if (
                            cur_params.get("target_pressure")
                            != current_params["target_pressure"]
                        ):
                            continue
                    ...
                    if prev_stage_params:
                        ...
                        if len(prev_params) != len(prev_stages_data):
                            ...
                            for i, prev_param in enumerate(prev_params):
                                ...
                                if (
                                    prev_param.get("pv") != expected_param["pv"]
                                    or prev_param.get("els") != expected_param["els"]
                                    or prev_param.get("element_type")
                                    != expected_param["element_type"]
                                ):
                                    prev_match = False
                                    break
                                ...
                                if stage_num == 1:
                                    if (
                                        prev_param.get("feed_pressure")
                                        != expected_param["feed_pressure"]
                                    ):
                                        prev_match = False
                                        break
                                else:
                                    if (
                                        prev_param.get("target_pressure")
                                        != expected_param["target_pressure"]
                                    ):
                                        prev_match = False
                                        break
                                    # Check boost pressure if provided
                                    if "boost_pressure" in expected_param and (
                                        "boost_pressure" not in prev_param
                                        or prev_param.get("boost_pressure")
                                        != expected_param["boost_pressure"]
                                    ):
                                        prev_match = False
                                        break
            ...
            if matched_row is None:
                ...
                logger.error(
                    f"No matching report found for stage {stage}, PV {pv}, ELS {els}, {pressure_field}: {pressure}"
                )
                return None, None
            ...
            element_flow_df = pd.read_excel(
                self.prev_stage_excel_file, sheet_name="element_flow"
            )
            ...
            if not matching_rows.any():
                logger.error(
                    f"No row found containing filename {filename} in element_flow sheet"
                )
                return None, None
            ...
            feed_press_columns.sort(
                key=lambda x: int(re.match(r"row_(\d+)_Feed_Press", x).group(1))
            )
            ...
            logger.info(
                f"Found feed pressure {rounded_feed_pressure} for stage {stage}"
            )

            # get the boost pressure
            boost_pressure = matched_row["current_stage_params"].get(
                "boost_pressure", None
            )
    ...
    def run_parameter_sweep(self, stage_configs):
            ...
            if self.stages == 1:
                ...
                for pv, els, pressure, cr_flow in itertools.product(
                    ...
                    if restart_counter >= restart_threshold:
                        logger.info(
                            f"Restarting WAVE application after {restart_counter} successful runs"
                        )
                        self.close_application()
                        time.sleep(5)  # Give the application time to close completely
                        if not self.launch_wave():
                            logger.error(
                                "Failed to restart WAVE. Aborting remaining parameter sweep."
                            )
                            break
                        restart_counter = 0
                ...
                current_pv_range = range(
                    current_config["pv_range"][0], current_config["pv_range"][1] + 1
                )
                current_els_range = range(
                    current_config["els_range"][0], current_config["els_range"][1] + 1
                )
                ...
                target_step = current_config.get(
                    "target_pressure_step", 1
                )  # Default to 1 if not specified

                # Generate target pressure values using step size
                target_values = [
                    round(target_start + i * target_step, 1)
                ...
                for stage in range(1, current_stage):
                    ...
                    pv_range = range(config["pv_range"][0], config["pv_range"][1] + 1)
                    els_range = range(
                        config["els_range"][0], config["els_range"][1] + 1
                    )
                    element_type = config["element_type"]
                    ...
                    stage_params = list(
                        itertools.product(
                            pv_range, els_range, (element_type,), pressure_values
                        )
                    )
                ...
                for prev_params in itertools.product(*prev_stage_combinations):
                    ...
                    prev_pv, prev_els, prev_ele_type, prev_pressure = prev_params[
                        prev_stage - 1
                    ]
                    ...
                    if prev_stage > 1:
                        prev_boost_pressure, feed_pressure = (
                            self.get_pressure_from_excel(
                                prev_stage,
                                prev_pv,
                                prev_els,
                                prev_ele_type,
                    ...
                    if feed_pressure is None:
                        pressure_type = (
                            "feed_pressure" if prev_stage == 1 else "boost_pressure"
                        )
                        logger.info(
                            f"Unable to find the previous stage results for the combination pv:{prev_pv} els:{prev_els} {pressure_type}: {prev_pressure}"
                        )
                        continue
                    ...
                    valid_targets = self.get_valid_target_pressures(
                        target_values, feed_pressure
                    )

                    if not valid_targets:
                        logger.info(
                            f"No valid target pressures for prev_stage params: {prev_params}"
                        )
                        continue
                    ...
                    for target, cr_flow in itertools.product(valid_targets, conc_recycle_flow):
                        ...
                        for pv, els in itertools.product(
                            current_pv_range, current_els_range
                        ):
                            ...
                            if restart_counter >= restart_threshold:
                                ...
                                time.sleep(
                                    5
                                )  # Give the application time to close completely
                                if not self.launch_wave():
                                    logger.error(
                                        "Failed to restart WAVE. Aborting remaining parameter sweep."
                                    )
                            ...
                            logger.info(
                                f"Current Stage{current_stage}: PV={pv}, ELS={els}, Element_Type={cur_element_type}, Target={target}, Boost={boost_pressure}"
                            )
        ...
        logger.info(
            f"Parameter sweep completed. Successful runs: {successful_runs}/{total_combinations}"
        )
        return successful_runs, total_combinations
